...
2. Host a shared test world and leave the supported game running for at least
   35 seconds. Keep Save Shift and another unrelated window open.
...
5. Confirm every offered candidate contains only the game window. Select one
   and verify it appears, cropped without distortion, on the world card.
...
9. Force the game into an exclusive-fullscreen mode that cannot be captured.
